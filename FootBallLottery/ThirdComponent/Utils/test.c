...
/* PROTO_LIST is defined depending on how PROTOTYPES is defined above.
 ...
 PROTO_LIST根据上面定义的原型是如何被定义。 如果使用的原型，然后PROTO_LIST返回列表，否则 返回一个空列表
 */
#if PROTOTYPES
...
char* hmac_md5(const char* text, const char* key);
...
/* MD5 block update operation. Continues an MD5 message-digest
 ...
 context.
 MD5块更新操作。继续MD5消息摘要 操作，处理其他的消息块，并更新 上下文。
 结构体对象，输入的字符串，输入的字符串的长度
 */
void MD5Update (MD5_CTX *context, unsigned char *input,unsigned int inputLen )
/* context */
/* input block */
/* length of input block */
{
    ...
    printf("%lu",context->count[0]);
    printf("%lu",context->count[1]);
    partLen = 64 - index;
