...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
    if(![[AppDelegate getAppDelegate] isSlider])
    {
        return;
    }
...
- (void)setMenuState:(MFSideMenuState)menuState {
    switch (menuState) {
        case MFSideMenuStateClosed:
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"viewControllerClosed" object:nil];
            [self closeSideMenu];
        }
            break;
        case MFSideMenuStateLeftMenuOpen:
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"removeGuide" object:nil];
            break;
        case MFSideMenuStateRightMenuOpen:
...
- (void)openRightSideMenu {
    [self.menuContainerView bringSubviewToFront:self.rightSideMenuViewController.view];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"rightSideMenViewController" object:nil];
...
//        [self setRootControllerOffset:(leftSideMenu ? self.menuWidth : -1*self.menuWidth)];
        [self setRootControllerOffset:(leftSideMenu ? self.menuWidth : -1*kWindowWidth)];
...
//    self.rightSideMenuViewController.view.frame = rightMenuFrame;
    //change by jaye
    self.rightSideMenuViewController.view.frame = CGRectMake(0, -20, kWindowWidth, kWindowHeight);
}
